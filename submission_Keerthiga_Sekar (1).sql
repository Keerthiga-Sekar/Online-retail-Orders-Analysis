...
JOIN ONLINE_CUSTOMER OC ON OH.CUSTOMER_ID = OC.CUSTOMER_ID
JOIN ADDRESS A ON OC.ADDRESS_ID = A.ADDRESS_ID
WHERE P.PRODUCT_ID IN (SELECT OI.PRODUCT_ID FROM ORDER_ITEMS OI WHERE OI.ORDER_ID IN 
...
AND OC.ADDRESS_ID NOT IN (SELECT ADDRESS_ID FROM ADDRESS WHERE PINCODE LIKE '5%')
